...
**Important**: This repository contains only this instruction file and the devcontainer configuration on first clone. All project files (pom.xml, source code, tests) must be generated by Copilot and tested before the project is complete.
...
After cloning this repository, Copilot must generate the following files:

1. **Build Configuration**
   - `pom.xml` - Maven build file with Spring Boot 3.5 and Java 21

2. **Application Source**
   - `src/main/java/com/example/helloworld/HelloWorldApplication.java` - Main application class
   - `src/main/java/com/example/helloworld/HelloController.java` - REST controller
   - `src/main/resources/application.properties` - Application configuration

3. **Test Files**
   - `src/test/java/com/example/helloworld/HelloControllerTest.java` - Unit tests (@WebMvcTest)
   - `src/test/java/com/example/helloworld/HelloControllerIntegrationTest.java` - Integration tests (@SpringBootTest + MockMvc)
   - `src/test/java/com/example/helloworld/HelloWorldApplicationTests.java` - Integration tests (@SpringBootTest + TestRestTemplate)

4. **Validation**
   - Run `mvn clean install` to verify all files compile and tests pass
   - Start the application and test endpoints with curl
...
### After Fresh Clone (Project Generation)
1. Check if `pom.xml` exists - if not, generate all project files
2. Generate files in this order:
   - `pom.xml` first (required for Maven)
   - Main application class (`HelloWorldApplication.java`)
   - Controller (`HelloController.java`)
   - Application properties
   - Test files
3. Run `mvn clean install` to compile and test
4. Fix any compilation or test failures before considering complete
